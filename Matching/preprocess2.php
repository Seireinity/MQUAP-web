<?php 
//Preprocessing Text: certification outcomes, certification topics
//                    course objectives, course topics
//opposite of Matching > preprocess.php
...
class Preprocess
{
    ...
    public function courseTQuery(){ //Course TOPICS
        ...
        $query = "SELECT topic
        FROM course_topics
        WHERE course_topics.certification_id = $id";
        $result = mysqli_query($db, $query);
    ...
    public function certLOQuery(){ //Certification OUTCOMES
        include 'connect-db.php';
        $id = $_GET["id"]; //CERTIFICATION id
    ...
    public function courseLOQuery(){ //COURSE OBJECTIVES
        include 'connect-db.php';
        $id = $_GET['id']; //CERTIFICATION id
    ...
    public function courseContentQuery(){ //for word2vec - COURSE CONTENT
        include 'connect-db.php';
        ...
        for($i=1; $i<$c+1; $i++){
            $query = "SELECT topic FROM course_topics
            WHERE course_id = $i
            UNION
            ...
            $text = strtolower($output2); //lowercase course content
            $output2 = $this->removeStopwords($text); //remove stopwords
            $cert = explode(" ", $output2); //revert back to array
            $courses[] = array_unique($cert); //2D array for word2vec
...
////////////////////COURSE Content for Cosine
$course = $preprocess->courseContentQuery();

$test = $preprocess->test(); //array of certification names and id only

foreach($course as $c){ //array of COURSE content
    $sample[] = implode(" ", $c); 
}

//COURSE input
for($i=0; $i<count($test); $i++){
        $y[]["course"] = $test[$i]['name'];
        $y[$i]["tags"] = $sample[$i]; //need to add tags to COURSE array
